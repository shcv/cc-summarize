...
class NoAISummarizer:
    ...
    def summarize_turn(self, turn: ConversationTurn, session_id: str = '') -> SummaryResult:
        ...
        for msg in turn.assistant_messages:
            if msg.tool_name:
                tool_calls.append(f"{msg.tool_name}: {self._format_tool_args(msg.tool_args)}")
        ...
        if not summary_parts and tool_calls:
            ...
            summary_parts = [f"Used tools: {', '.join([t.split(':')[0] for t in tool_calls[:5]])}"]
        
        final_summary = "\n".join(summary_parts) if summary_parts else "No summary information available in logs."
    ...
    def _is_todo_activity(self, msg: Message) -> bool:
        ...
        todo_indicators = [
            'todowrite', 'todo', 'task', 'working on', 'implementing',
            'starting', 'completing', 'finished', 'adding', 'creating'
        ]
    ...
    def _extract_todo_content(self, msg: Message) -> str:
        ...
        if 'todowrite' in content.lower():
            ...
            # Generic cleanup
            cleaned = content.replace('\u001b[1m', '').replace('\u001b[22m', '')  # Remove bold codes
    ...
    def summarize_session(self, turns: List[ConversationTurn], session_id: str = '') -> List[SummaryResult]:
...
class UserOnlyExtractor:
    ...
    def _is_system_noise(self, content: str) -> bool:
        ...
        if (content.startswith('<command-') or 
            content.startswith('<local-command-') or
            'command-message' in content or
            '[Request interrupted' in content):
            return True
...
class MessageExtractor:
    ...
    def extract_messages(self, turns: List[ConversationTurn], categories: List[str] = None) -> List[dict]:
        ...
        for turn in turns:
            ...
            if (turn.user_message.message_category in categories and 
                turn.user_message.message_category not in excluded_categories):
            ...
            for assistant_msg in turn.assistant_messages:
                if (assistant_msg.message_category in categories and
                    assistant_msg.message_category not in excluded_categories):
            ...
            for item in content:
                if isinstance(item, dict):
                    if item.get('type') == 'text':
                        ...
                        plan_input = item.get('input', {})
                        if 'plan' in plan_input:
                            parts.append(plan_input['plan'])
                        else:
                            parts.append(str(item))
                    else:
                        # Other content types
